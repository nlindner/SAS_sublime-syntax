...
proc means 
   data=work.cake chartype nway noprint;
...
proc sql feedback stimer nofeedback _method;
   connect to oledb  (
   );
...
%macro testme(out_TableNm=work.summary);
   %local Comparison_FieldList thisItem i nItem;
   %let Comparison_FieldList = MM_Risk_Base MM_Risk_Only Auths_Count Admit_Count Utilization;

   %let nItem = %countItemInList(&Comparison_FieldList);
   ...
   data &out_TableNm;
      ...
      attrib 
         %do i = 1 %to &nItem;
            %let thisItem = %upcase(%getItemInString(&Comparison_FieldList, &i));
            %if &ThisItem = MM_RISK_BASE
            %then %do;
               label MM_RISK_BASE = "Member Months_Diff";
            %end;
         %end;
      ;
   run;
